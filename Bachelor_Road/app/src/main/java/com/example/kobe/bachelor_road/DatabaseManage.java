...
public class DatabaseManage {
    ...
    //修改活力值上限
    public String updateCHMaximumEnergy(int CHMaximumEnergy){
        SQLiteDatabase sQLiteDatabase=database.getReadableDatabase(); //以读写方式打开数据库
        ContentValues values = new ContentValues();
        values.clear();
    ...
    public CharacterCourse[] queryCharacterCourse(int week){
        ...
        for (int i=0; cursor.moveToNext() == true; i++ ){
            ...
            characterCourse[i].CHCClassLocationBuild = cursor.getString(cursor.getColumnIndex("CHCClassLocationBuild"));
            characterCourse[i].CHCClassLocationClass = cursor.getString(cursor.getColumnIndex("CHCClassLocationClass"));
            characterCourse[i].CHCIsAttendClass = cursor.getInt(cursor.getColumnIndex("CHCIsAttendClass")) > 0;
        }
        sQLiteDatabase.close();
